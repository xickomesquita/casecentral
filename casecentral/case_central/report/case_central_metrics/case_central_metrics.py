...
def get_applicable_calendar_years():
    # Fetch all fiscal years and compute min/max in Python
    fiscal_years = frappe.get_all("Fiscal Year", fields=["year_start_date", "year_end_date"])
    if not fiscal_years:
        return [], getdate(nowdate()).year
    start_year = min(getdate(fy["year_start_date"]).year for fy in fiscal_years)
    end_year = max(getdate(fy["year_end_date"]).year for fy in fiscal_years)
...
def get_period_list(
    ...
    if filter_based_on == "Fiscal Year":
        ...
        fiscal_years = frappe.get_all("Fiscal Year", fields=["year_start_date", "year_end_date"])
        if not fiscal_years:
            # fallback to calendar years if no fiscal years exist
            year_start_date = getdate(datetime.date(int(from_year), 1, 1))
            year_end_date = getdate(datetime.date(int(to_year), 12, 31))
        else:
            min_year = min(getdate(fy["year_start_date"]) for fy in fiscal_years)
            max_year = max(getdate(fy["year_end_date"]) for fy in fiscal_years)
            if int(from_year) == min_year.year:
                year_start_date = min_year
            else:
                year_start_date = getdate(datetime.date(int(from_year), 1, 1))
            if int(to_year) == max_year.year:
                year_end_date = max_year
            else:
                year_end_date = getdate(datetime.date(int(to_year), 12, 31))
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
    # Fetch all fiscal years in the range and compute min/max in Python
    fiscal_years = frappe.get_all(
        "Fiscal Year",
        filters={"name": ["between", [from_fiscal_year, to_fiscal_year]]},
        fields=["year_start_date", "year_end_date"]
    )
    if not fiscal_years:
        return {}
    year_start_date = min(getdate(fy["year_start_date"]) for fy in fiscal_years)
    year_end_date = max(getdate(fy["year_end_date"]) for fy in fiscal_years)
    return {"year_start_date": year_start_date, "year_end_date": year_end_date}
